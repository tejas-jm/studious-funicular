...
def extract_skills(tokens: List[Token]) -> List[str]:
    text = _join_tokens(tokens)
    if not text:
        return []
...
def build_projects(tokens: List[Token]) -> List[Project]:
    ...
    for segment in segments:
        ...
        desc = segment[len(name) :].strip(" -") if len(segment) > len(name) else None
        technologies = [
            tech.strip()
            for tech in re.findall(r"\b[A-Za-z][A-Za-z0-9+#/.\s]{0,30}", desc or "")
            if tech.strip()
        ]
        projects.append(Project(name=name or None, description=desc or None, technologies=technologies))
    return projects
...
def build_publications(tokens: List[Token]) -> List[Publication]:
    ...
    for line in lines:
        ...
        publications.append(Publication(title=cleaned or None, url=url))
    return publications
...
def build_languages(tokens: List[Token]) -> List[Language]:
    ...
    for entry in entries:
        ...
        languages.append(Language(language=language or None, fluency=fluency or None))
    return languages
...
def link_entities(
    ...
    if embeddings:
        resume.meta["token_embeddings"] = len(embeddings)
